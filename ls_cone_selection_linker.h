...
#ifndef __H__UG__PLUGINS__LEVEL_SET_CONES__
#define __H__UG__PLUGINS__LEVEL_SET_CONES__
...
namespace ug
{
   namespace NeuroGrowth
   {
      ...
      class LSConesSelection
          : public StdDataLinker<LSConesSelection<TDomain, TAlgebra>, MathVector<TDomain::dim>, TDomain::dim>
      {
         //	domain type
         ...
         //	Base class type
         typedef StdDataLinker<LSConesSelection<domain_type, algebra_type>, MathVector<dim>, dim> base_type;

         //	extrapolation type
         typedef IInterfaceExtrapolation<domain_type, algebra_type> extrapol_type;

      public:
         LSConesSelection() : m_spExtrapolation(NULL),
                               m_spTubuline(NULL), m_spDTubuline(NULL),
                               m_spMagnitudFlux(NULL),
                               m_spEikanolGrad(NULL),
      ...
      public:
         ...
         {
            UG_THROW("LSConesSelection: Element is necessary for the evaluation.");
         }
                              ...
                              const MathMatrix<refDim, dim> *vJT = NULL) const
         {
            ...
            std::vector<MathVector<dim>> vEikanolGrad(nip);
            ...
            (*m_spLevelSetGrad)(&vLevelSetGrad[0], vGlobIP, time, si,
                                elem, vCornerCoords, vLocIP, nip, u, vJT);
            (*m_spEikanolGrad)(&vEikanolGrad[0], vGlobIP, time, si,
                        elem, vCornerCoords, vLocIP, nip, u, vJT);             
            ...
            for (size_t ip = 0; ip < nip; ++ip)
            {
               ...
               number flux = MagnitudFlux * -1;  // cantidad de concentración que entrará por las paredes
                             ...
                             const MathMatrix<refDim, dim> *vJT = NULL) const
         {
            ...
            for (size_t ip = 0; ip < nip; ++ip)
            {
               ...
               //	Calculate Direccion
               MathVector<dim> Eikanol;                                              // vector to save direction of the velocity
               number gradientnorm2 = VecLength(vEikanolGrad[ip]);                     // obtain the norm of the gradient of the LevelSet
               VecScale(Eikanol, vEikanolGrad[ip], 1.0 / (gradientnorm2 + 0.00001)); // normalize the gradient of the LevelSet


               // Magnitud of the flux
               number flux =  MagnitudFlux * -1;  // cantidad de concentración que entrará por las paredes

               number prodcuto = VecDot(direccion, Eikanol); // producto entre la dirección del crecimiento y la dirección del eikanol
      ...
      public:
         ...
         ///	set LevelSet gradient import
         void set_Eikanol_gradient(SmartPtr<CplUserData<MathVector<dim>, dim>> data)
         {
            m_spEikanolGrad = data;
            // m_spDLevelSetGrad = data.template cast_dynamic<DependentUserData<MathVector<dim>, dim>>();
            // base_type::set_input(_DP_, data, data);
         }
      ...
      protected:
         ...
         SmartPtr<CplUserData<MathVector<dim>, dim>> m_spEikanolGrad;
...
#endif /* __H__UG__PLUGINS__LEVEL_SET_CONES__ */
