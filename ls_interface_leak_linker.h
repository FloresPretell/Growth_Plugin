...
#ifndef __H__UG__PLUGINS__LS_INTERFACE_LEAK_LINKER__
#define __H__UG__PLUGINS__LS_INTERFACE_LEAK_LINKER__
...
namespace ug
{
   namespace NeuroGrowth
   {
      ...
      class LSInterfaceLeak
          : public StdDataLinker<LSInterfaceLeak<TDomain, TAlgebra>, MathVector<TDomain::dim>, TDomain::dim>
      {
         //	domain type
         ...
         //	Base class type
         typedef StdDataLinker<LSInterfaceLeak<domain_type, algebra_type>, MathVector<dim>, dim> base_type;

         //	extrapolation type
         typedef IInterfaceExtrapolation<domain_type, algebra_type> extrapol_type;

      public:
         LSInterfaceLeak() : m_spExtrapolation(NULL),
                               m_spTubuline(NULL), m_spDTubuline(NULL),
                               m_spMagnitudFlux(NULL), m_spCaExt(-1.0),
      ...
      public:
                              ...
                              const MathMatrix<refDim, dim> *vJT = NULL) const
         {
            ...
            for (size_t ip = 0; ip < nip; ++ip)
            {
               ...
               // [leak BC] if CaExt>=0: flux = -v_leak*(CaExt - Ca_cyt) (inward when CaExt>Ca_cyt),
               // i.e. neuro_collection Leak form; else legacy constant influx.
               number flux;
               if (m_spCaExt >= 0.0)
                  flux = -MagnitudFlux * (m_spCaExt - vTubuline[ip]);
               else
                  flux = MagnitudFlux * -1;  // cantidad de concentración que entrará por las paredes
                             ...
                             const MathMatrix<refDim, dim> *vJT = NULL) const
         {
            ...
            for (size_t ip = 0; ip < nip; ++ip)
            {
               ...
               // Magnitud of the flux ([leak BC], see evaluate())
               number flux;
               if (m_spCaExt >= 0.0)
                  flux = -MagnitudFlux * (m_spCaExt - vTubuline[ip]);
               else
      ...
      public:
         ...
         void set_domain_discretizacion(SmartPtr<extrapol_type> spExtrapol)
         {
			...
			///	[leak BC] set external Ca concentration -> flux = magnitud*(CaExt - Ca_cyt)
			///	(neuro_collection Leak form). Negative value keeps the legacy constant-influx mode.
			void set_CaExt(number data)
			{
				m_spCaExt = data;
			}
      ...
      protected:
         ...
         number m_spCaExt;   // [leak BC] external Ca; <0 => legacy constant-influx mode
      };

   } // end namespace NeuroGrowth
} // end namespace ug

#endif /* __H__UG__PLUGINS__LS_INTERFACE_LEAK_LINKER__ */
