...
function GenerateImaginaryMolecule2(ApproxSpace_lsf, gridfunction_gradiente, gridfunction_ls, domain, inicial_pregunta)
    print("------------------------------------------------------------------------------------------")
    print("3.1.1) Start: Eikanol inside")
    print("------------------------------------------------------------------------------------------")
    ...
    local geo_solverDesc3D = { --- yes
        ...
        linSolver = {
            ...
            convCheck = {
                ...
                reduction = 1e-8, -- reduction factor of defect to be reached
...
function GenerateImaginaryMolecule(ApproxSpace_lsf, gridfunction_ls, domain)
    print("\n\n ------------------------------------------------------------------------------------------")
    print("3.1.2) Start: Stationary Difusion inside")
    print("------------------------------------------------------------------------------------------")
    ...
    flujo:set_magnitud_influx(-1) -- - 1.0
    ...
    -- local solverDesc = { --- yes
    --     type = "newton", -- nombre del metodo de discretization 
    
    --     lineSearch = {
    --         type = "standard", -- ["standard", "none"]
    --         maxSteps = 15, -- maximum number of line search steps
    --         lambdaStart = 1, -- start value for scaling parameter
    --         lambdaReduce = 0.7, -- reduction factor for scaling parameter
    --         acceptBest = true, -- check for best solution if true
    --         checkAll = false -- check all maxSteps steps if true 
    ...
    --         type = "standard",
    --         iterations = 50, -- maximum number of iterations
    --         absolute = 5e-8, -- absolut value of defect to be reached; usually 1e-7 - 1e-9
...
function SmoothCutrvature(ApproxSpace_lsf, gridfunction_curvature, domain, Difusion_smooth)
    ...
    --- Domain Discretization
    ----------------------------------------------------------------------------
    SmoothCurvature_Imp = ConvectionDiffusionFV1("ca_cyt", "Outer") -- DIFUSION PROCESS
    ...
    --         type = "gmg", -- preconditioner ["gmg", "ilu", "ilut", "jac", "gs", "sgs"]
    --         approxSpace = approxSpace,
    --         smoother = "sgs", -- gmg-smoother ["ilu", "ilut", "jac", "gs", "sgs"]
    --         cycle = "V", -- gmg-cycle ["V", "F", "W"]
    --         preSmooth = 3, -- number presmoothing steps
    --         postSmooth = 3, -- number postsmoothing steps,
    --         rap = true, -- use the Galerkin formular in the hierarchy
    ...
    local geo_endTime = geo_NumTimeSteps * geo_dt

    util.SolveLinearTimeProblem(curvature, {
        domainDisc = CurvatureDomainDisc,
        reassemble = true
    }, solver2, nil, nil, "ImplEuler", 1, 0, geo_endTime, geo_dt, min_dt , 0.5)
    print("Finish: isotropic smoothing of the curvature")
    return curvature
end
...
function SmoothCutrvatureAnisotropic(ApproxSpace_lsf, gridfunction_ls, gridfunction_curvature, domain , Difusion_smooth)
    print("\n\n ------------------------------------------------------------------------------------------")
    print("-------- Start with anisotropic smoothing -----------------------------------")
    print("------------------------------------------------------------------------------------------")
    ...
    util.SolveNonlinearTimeProblem(curvature, CurvatureDomainDisc, solver2, nil, nil, "ImplEuler", 1, 0, geo_endTime, geo_dt, min_dt , 0.5)

    print("Finish: anisotropic smoothing of the curvature")
    return curvature
end
...
function Smooth_Normal_Interface_Velocity(ApproxSpace_lsf, gridfunction_normal_velocity, domain, Difusion_smooth)
    print("\n\n ------------------------------------------------------------------------------------------")
    print("-------- Start with isotropic smoothing of interface velocity -----------------------------------")
    print("------------------------------------------------------------------------------------------")
    -- 2. My inputs-----------------------------------------------------------------
    local dom = domain
    local approxSpace = ApproxSpace_lsf
    ...
    --- Domain Discretization
    ----------------------------------------------------------------------------
    SmoothVelNorm_Imp = ConvectionDiffusionFV1("ca_cyt", "Outer") -- DIFUSION PROCESS
    ...
    print("Finish: isotropic smoothing of the velocity of the interface")
...
function GenerateInitialInhibition (num_Steps_Initial_inh, gridfunction_inhibition)
    ...
    print("Finished the initial value: outside the neuron ....")
    return gridfunction_inhibition
end
