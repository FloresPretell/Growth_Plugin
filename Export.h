 ...
 */
#ifndef __H__UG__PLUGINS__LEVEL_SET__EXPORT_DISCR_H__
#define __H__UG__PLUGINS__LEVEL_SET__EXPORT_DISCR_H__
...
namespace ug
{
   namespace NeuroGrowth
   {
      ...
      public:
         ...
         ///	Copy the vector in grid_function
         void Save_Linker_to_Gridfuntion(
             SmartPtr<CplUserData<MathVector<dim>, dim>> spVelField,
             SmartPtr<TGridFunction> spNormVel);

         ///	Copy the vector in grid_function
         void Save_Linker_to_Gridfuntion_by_component(
             SmartPtr<CplUserData<MathVector<dim>, dim>> spVelField,
             SmartPtr<TGridFunction> spNormVel,
             const char* fct);    
...
#define __H__UG__PLUGINS__LEVEL_SET__EXPORT
       ...
       */
      template <typename TGridFunction>
      int Export<TGridFunction>::lsf_sign(
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::mark_CoIE(
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::extrapolate_by_lsf(
       ...
       */
      template <typename TGridFunction>
      inline void Export<TGridFunction>::sol_update(
       ...
       */
      template <typename TGridFunction>
      inline void Export<TGridFunction>::bnd_sol_update(
          bool redOrder,               ///< whether to reduce the order of the discretization
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::get_nodal_vel(
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::assemble_element(
          ElemType *elem,           ///< the element to compute the contribution for
          DimFV1Geometry<dim> &geo, ///< structure for the FV geometry
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::get_scvf_vel_on_if(
          DimFV1Geometry<dim> &geo,                       ///< structure for the FV geometry
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::get_bf_vel_on_if(
          DimFV1Geometry<dim> &geo,                   ///< structure for the FV geometry
       ...
       */
      template <typename TGridFunction>
      int Export<TGridFunction>::assemble_cut_element(
          ElemType *elem,                        ///< the element to compute the contribution for
          DimFV1Geometry<dim> &geo,              ///< structure for the FV geometry
          domain_type &domain,                   ///< domain
          LocalVector &uOld,                     ///< solution on the old time level
...
#ifndef __H__UG__PLUGINS__LEVEL_SET__EXPORT
...
#else // __H__UG__PLUGINS__LEVEL_SET__EXPORT
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::compute_volumes(
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::compute_elem_grad(
          DimFV1Geometry<dim> &geo,              ///< FV geometry
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::compute_vertex_grad(
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::assign_dirichlet(
          TGridFunction &numsol)
      {
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::limit_grad(
      ...
      /**
       * Copy the velocity of vector in a gridfunction
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::Save_Linker_to_Gridfuntion(
      ...
      {
         m_spLSF = spNormVel; // this is only because they need a gridfuntion to obtain the spatial discretization, thos code was obtained from compute_normal_velocity, and it has gridfunction mpLSF : level set
         //	we need the solution
         if (!m_spLSF.valid())
            UG_THROW("Specify the gridfunction to save the velocity from the linker.");
         ...
         for (int si = 0; si < m_spLSF->num_subsets(); si++)
         {
            ...
            for (ElemIterator iter = m_spLSF->template begin<ElemType>(si); iter != iterEnd; ++iter)
            {
               ...
               MathVector<dim> co_vel[maxNumCo]; // velocidad en los nodos
               ...
               for (size_t i = 0; i < noc; i++)
               {
                  ...
                  for (int d = 0; d < dim; d++)
                  {
                     spNormVel->inner_dof_indices(vrt, d, ind);
                     DoFRef(*spNormVel, ind[0]) = co_vel[i][d];
                  }
               }
            }
         }
 ...
 //        spNormVel->change_storage_type(PST_CONSISTENT);
         spNormVel->set_storage_type(PST_CONSISTENT);
       ...
       */
      template <typename TGridFunction>
      void Export<TGridFunction>::Save_Linker_to_Gridfuntion_by_component(
          SmartPtr<CplUserData<MathVector<dim>, dim>> spVelField, ///< user data for the velocity vector field
          SmartPtr<TGridFunction> spNormVel,                     ///< to save the normal velocity
          const char* fct                                             /// Component to inicialize (the approximation space have a lot of componets )
      )
      {


         m_spLSF = spNormVel; // this is only because they need a gridfuntion to obtain the spatial discretization, thos code was obtained from compute_normal_velocity, and it has gridfunction mpLSF : level set
         const int fct1 = m_spLSF->fct_id_by_name(fct);
         ...
         for (int si = 0; si < m_spLSF->num_subsets(); si++)
         {
            ...
            for (ElemIterator iter = m_spLSF->template begin<ElemType>(si); iter != iterEnd; ++iter)
            {
               ...
               {
                  Vertex *vrt = vVrt[i];
                  spNormVel->inner_dof_indices(vrt, fct1, ind); // (vrt, fct, ind)
                  DoFRef(*spNormVel, ind[0]) = co_vel[i][0]; //co_vel[i][fct]
...
#endif /* __H__UG__PLUGINS__LEVEL_SET__EXPORT_DISCR_H__ */
