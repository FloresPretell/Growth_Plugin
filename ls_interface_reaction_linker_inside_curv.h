...
#ifndef __H__UG__PLUGINS__LEVEL_SET_INTERFACE_REACTION_INSIDE_LINKER__
#define __H__UG__PLUGINS__LEVEL_SET_INTERFACE_REACTION_INSIDE_LINKER__
...
namespace ug
{
   namespace NeuroGrowth
   {
      ...
      class LSReactionInsideCurv
          : public StdDataLinker<LSReactionInsideCurv<TDomain, TAlgebra>, number, TDomain::dim>
      {
         //	domain type
         ...
         //	Base class type
         typedef StdDataLinker<LSReactionInsideCurv<domain_type, algebra_type>,  number, dim> base_type;

         //	extrapolation type
         typedef IInterfaceExtrapolation<domain_type, algebra_type> extrapol_type;

      public:
         LSReactionInsideCurv() : m_spExtrapolation(NULL),
      ...
      public:
                              ...
                              const MathMatrix<refDim, dim> *vJT = NULL) const
         {
            ...
            if (elem_cut(elem, si, vCornerCoords, time) < 0 ) //  outside (<0)  the free surface
            {
            ...
            for (size_t ip = 0; ip < nip; ++ip)
            {
               if (vCurvature[ip] > minimo ) // MINIMO ES 11 ( para que sea en el centro)
               {   
                  // Computamos el valor de reacción
                  //const number cuadratic = (vFirstTerm[ip] * vFirstTerm[ip]) / (Constantdiv + vFirstTerm[ip] * vFirstTerm[ip]);
                  number flux = 1 - (vFirstTerm[ip] / Constantdiv);
                  number flux2 = flux * vFirstTerm[ip];
                  vValue[ip] = flux2 * MagnitudFlux;
                             ...
                             const MathMatrix<refDim, dim> *vJT = NULL) const
         {
            ...
            if (elem_cut(elem, si, vCornerCoords, time) < 0 ) //  outside (<0) and inside (>0) the free surface
            {
               for (size_t ip = 0; ip < nip; ++ip)
               {
                    vValue[ip] = 0.0;
...
#endif /*__H__UG__PLUGINS__LEVEL_SET_INTERFACE_REACTION_INSIDE_LINKER__ */
