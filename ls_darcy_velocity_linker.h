...
#ifndef __H__UG__PLUGINS__LEVEL_SET_DARCY_VELOCITY_LINKER__
#define  __H__UG__PLUGINS__LEVEL_SET_DARCY_VELOCITY_LINKER__
 ...
 * 
 * Note that the input parameters should be properly extrapolated.
 * This linker merely cuts off the velocity above the interface.
 */
template <typename TDomain, typename TAlgebra>
class LSDarcyVelocityLinker
	: public StdDataLinker<LSDarcyVelocityLinker<TDomain, TAlgebra>, MathVector<TDomain::dim>, TDomain::dim>
{
	//	domain type
	...
	//	Base class type
		typedef StdDataLinker<LSDarcyVelocityLinker<domain_type, algebra_type>, MathVector<dim>, dim> base_type;
	...
	//	checks whether the element is cut
		...
		) const
		{
			if(m_spExtrapolation.valid())
			{
				...
				return ((extrapol_type *) m_spExtrapolation.get())->check_elem_lsf
					(rRefElem.num(0), elem, si, false, vCornerCoords, time);
			}			
			return 1;
		}
	...
	public:
		                     ...
		                     const MathMatrix<refDim, dim>* vJT = NULL) const
		{
			...
			{
				for(size_t ip = 0; ip < nip; ++ip) vValue[ip] = 0.0;
				return;
			}
		                    ...
		                    const MathMatrix<refDim, dim>* vJT = NULL) const
		{
			if (elem_cut (elem, si, vCornerCoords, time) < 0) // if above the free surface
			{
				for(size_t ip = 0; ip < nip; ++ip) vDarcyVel[ip] = 0.0;
