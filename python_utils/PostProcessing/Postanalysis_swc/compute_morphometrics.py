...
def main():
    ...
    if not base_path.is_dir():
        raise SystemExit(f"ERROR: Input path does not exist: {base_path}")

    pattern = os.path.join(str(base_path), "**", "clean_skeleton.swc")
    swc_files = sorted(glob.glob(pattern, recursive=True))

    if not swc_files:
        raise SystemExit(f"ERROR: no clean_skeleton.swc files found under {base_path}")
    ...
    if not rows:
        raise SystemExit("ERROR: No valid SWC files processed.")
