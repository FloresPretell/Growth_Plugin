...
LIST=/tmp/claude-194965/-scratch-flore0a/6bb62a79-db00-4d29-b3cd-d11bf40ec0c6/scratchpad/rest_dirs.txt
inodes(){ lfs quota -u flore0a /scratch 2>/dev/null | awk '/\/scratch/{print $6}'; }
echo "START rest-sweep inodes=$(inodes)  $(date)  ($(wc -l < $LIST) dirs)"
i=0; n=$(wc -l < $LIST)
...
echo "=== REST DONE $(date)  inodes=$(inodes) ==="
