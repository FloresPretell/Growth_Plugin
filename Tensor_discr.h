 ...
 */
#ifndef __H__UG__PLUGINS__LEVEL_SET__TENSOR_DISCR_H__
#define __H__UG__PLUGINS__LEVEL_SET__TENSOR_DISCR_H__
...
namespace ug
{
   namespace NeuroGrowth
   {
      ...
      public:
         ...
         void ComputeDiffusionTensorBeltrami(
             SmartPtr<TGridFunction> spLSF,
             SmartPtr<CplUserData<MathMatrix<dim, dim>, dim> > spDiffTensor);
...
#define __H__UG__PLUGINS__LEVEL_SET__KAPPLA
       ...
       */
      template <typename TGridFunction>
      int TensorDiffusion_LS<TGridFunction>::lsf_sign(
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::mark_CoIE(
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::extrapolate_by_lsf(
       ...
       */
      template <typename TGridFunction>
      inline void TensorDiffusion_LS<TGridFunction>::sol_update(
       ...
       */
      template <typename TGridFunction>
      inline void TensorDiffusion_LS<TGridFunction>::bnd_sol_update(
          bool redOrder,               ///< whether to reduce the order of the discretization
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::get_nodal_vel(
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::assemble_element(
          ElemType *elem,           ///< the element to compute the contribution for
          DimFV1Geometry<dim> &geo, ///< structure for the FV geometry
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::get_scvf_vel_on_if(
          DimFV1Geometry<dim> &geo,                       ///< structure for the FV geometry
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::get_bf_vel_on_if(
          DimFV1Geometry<dim> &geo,                   ///< structure for the FV geometry
       ...
       */
      template <typename TGridFunction>
      int TensorDiffusion_LS<TGridFunction>::assemble_cut_element(
          ElemType *elem,                        ///< the element to compute the contribution for
          DimFV1Geometry<dim> &geo,              ///< structure for the FV geometry
          domain_type &domain,                   ///< domain
          LocalVector &uOld,                     ///< solution on the old time level
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::compute_volumes(
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::compute_elem_grad(
          DimFV1Geometry<dim> &geo,              ///< FV geometry
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::compute_vertex_grad(
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::assign_dirichlet(
          TGridFunction &numsol)
      {
       ...
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::limit_grad(
      ...
      /**
       * Compute Kappla (curvature), using level-set function and save it in the grid function spDiffTensor
       */
      template <typename TGridFunction>
      void TensorDiffusion_LS<TGridFunction>::ComputeDiffusionTensorBeltrami(
          SmartPtr<TGridFunction> spLSF,   ///< level-set function
          SmartPtr<CplUserData<MathMatrix<dim, dim>, dim> > spDiffTensor ///< to save the normal velocity
      )
      {
         m_spLSF = spLSF;
         ...
         for (int si = 0; si < m_spLSF->num_subsets(); si++) // for SUBSET
         {
            ...
            for (ElemIterator iter = m_spLSF->template begin<ElemType>(si); iter != iterEnd; ++iter) // FOR ELEMENT
            {
               ...
               std::vector<MathMatrix<dim, dim> > vDiffTensor(n_scvf);
               ...
               // Normalize the gradient
                  ...
                  MathVector<dim> normal = Gradiente;
                  VecScale(normal, normal, 1.0 / gnorm);

                    MathMatrix<dim, dim> I;
                    MathMatrix<dim, dim> NormalProj;
                    MathMatrix<dim, dim> DiffTensor;
                    number diffusion_coeff = 1.0; 
                    ...
                    vDiffTensor[ip] = DiffTensor;

               }

                            // Llamar a la función de usuario para calcular el tensor
                (*spDiffTensor)(&vDiffTensor[0], geo.scvf_global_ips(), time, si,
                                elem, coCoord, geo.scvf_local_ips(), n_scvf, NULL);
