...
/*
 * Extrapolation over the embedded boundary based on the linearization of the
 * function to extrapolate by a linear function in the element.
 */
#ifndef __H__UG__PLUGINS__LEVSET_LIN_EXTRAPOL__
#define __H__UG__PLUGINS__LEVSET_LIN_EXTRAPOL__
...
///	prepares the boundary conditions at the interface: sets all them to Dirichlet-0
	...
	{
		size_t old_num_fct = m_vICData.size ();
		size_t num_fct = spApproxSpace->num_fct ();
		m_vICData.resize (num_fct);
		for (size_t fct = old_num_fct; fct < num_fct; fct++)
		{
			m_vICData[fct].Dirichlet = true;
			m_vICData[fct].func = SPNULL;
			m_vICData[fct].value = 0;
			m_vICData[fct].excl_ss.set_subset_handler (spApproxSpace->subset_handler ());
		}
	}
...
///	Class for the specification of the boundary conditions at the interface (for one component/function)
	struct ICData
	{
		bool Dirichlet; ///< false for Neumann-0 BC, true for Dirichlet BC
		SmartPtr<CplUserData<number, dim> > func; ///< variable Dirichlet value (for the Dirichlet BC)
		number value; ///< Dirichlet value (for the Dirichlet BC, if func is NULL)
		SubsetGroup excl_ss; ///< subsets to exclude
	};
...
//	temporaty data (valid for the initialization with a particular element)
	...
	MathVector<dim> m_elemCenter; ///< center of the element
...
/**
 * Global assembler for the simple extrapolation method based on the LevSetGFlinearExtrapolation
 * class.
 *
 * \tparam TDomain		domain type
...
:	public LSGFDomainDiscretization<TDomain, TAlgebra, LevSetGFlinearExtrapolation<TDomain, TAlgebra> >
{
///	Type of the base class
	typedef LSGFDomainDiscretization<TDomain, TAlgebra, LevSetGFlinearExtrapolation<TDomain, TAlgebra> > base_type;
...
#include "levset_lin_extrapol_impl.h"

#endif // __H__UG__PLUGINS__LEVSET_LIN_EXTRAPOL__
