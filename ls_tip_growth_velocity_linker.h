...
#ifndef __H__UG__PLUGINS__LEVEL_SET_TIP_GROWTH_VELOCITY_LINKER__
#define __H__UG__PLUGINS__LEVEL_SET_TIP_GROWTH_VELOCITY_LINKER__
...
namespace ug
{
	namespace NeuroGrowth
	{
		...
		class LSTipGrowthVelocity
			 : public StdDataLinker<LSTipGrowthVelocity<TDomain, TAlgebra>, MathVector<TDomain::dim>, TDomain::dim>
		{
			//	domain type
			...
			//	Base class type
			typedef StdDataLinker<LSTipGrowthVelocity<domain_type, algebra_type>, MathVector<dim>, dim> base_type;

			//	extrapolation type
			typedef IInterfaceExtrapolation<domain_type, algebra_type> extrapol_type;

		public:
			LSTipGrowthVelocity(SmartPtr<extrapol_type> spExtrapol) : 
		...
		public:
			...
			{
				UG_THROW("LSTipGrowthVelocity: Element is necessary for the evaluation.");
			}
										...
										const MathMatrix<refDim, dim> *vJT = NULL) const
			{
				...
				// [TIP GROWTH] edad de la membrana (opcional)
				std::vector<number> vAgeBuf(nip, 0.0);
				if (m_spAge.valid())
					(*m_spAge)(&vAgeBuf[0], vGlobIP, time, si, elem, vCornerCoords, vLocIP, nip, u, vJT);
				const number *vAge = &vAgeBuf[0];
				...
				for (size_t ip = 0; ip < nip; ++ip)
				{
					...
					//
					// POR QUE: el gate  min < kappa < max  es artificial y ademas ROMPIBLE:
					// al engordar, kappa = 1/R baja, la punta cae fuera del gate, la velocidad
					// se vuelve CERO exacto y la rama se congela como bola. Es una trampa
					// monotona (engordar impide dejar de engordar).
					// En biologia --raices, tubos polinicos, hifas-- el crecimiento es APICAL:
					// no se elige donde crecer por la FORMA, sino porque (a) el material solo
					// llega a la punta y (b) la pared solo es blanda mientras es joven.
					//
					// Se sustituye el gate por dos factores con sentido fisico:
					//
					//   ENTREGA    (d.n)^q   d = direccion de transporte (eje de la rama)
					//                        n = normal de la interfaz
					//     En la punta el eje y la normal son paralelos -> d.n ~ 1 -> crece.
					//     En el flanco son perpendiculares -> d.n ~ 0 -> NO crece.
					//     La localizacion apical sale de la GEOMETRIA DEL TRANSPORTE, no de un
					//     umbral: no hay nada que desajustar cuando la forma cambia.
					//
					//   MADURACION  1/(1 + edad/tau)
					//     La membrana recien creada es blanda; al envejecer se rigidiza y deja
					//     de responder. Un trozo viejo no crece PORQUE ES VIEJO, no porque su
					//     curvatura cayo. Esto es lo que mantiene el tubo fino detras del apice.
					//
					// Se conserva la quimica: v ~ tubulina_efectiva * MAP2-ligado, y MAP2-ligado
					// es justo donde entra el calcio. Tubulina y B interactuan como antes.
					// set_use_curvature_gate(true) reactiva el gate para poder comparar.
					// =====================================================================
					const bool _gateOK = (!m_useGate) || (vCurvature[ip] > minimo && vCurvature[ip] < maximo);
					if (_gateOK)
               		{
						if (vTubuline[ip] > minimoTubulin)
						{
							...
							// --- FACTOR 1: TRASLACION QUE CONSERVA LA FORMA  v_n = V cos(theta) ---
							// IDENTIDAD CINEMATICA EXACTA, no una hipotesis de modelado.
							// Si una superficie se traslada rigidamente con velocidad V a lo largo
							// de e_z, entonces phi(x,t) = phi0(x - V t e_z) y al sustituir en
							// phi_t + F|grad phi| = 0 sale
							//        F = V (e_z . n)  =  V cos(theta)
							// (Sethian 2001, JCP, ec. 5-6). Cualquier velocidad normal que NO cumpla
							// esto no puede trasladar la forma: la deforma. Aqui e_z es la direccion
							// LOCAL de transporte (el eje de la rama), asi que cos(theta) = d . n.
							//
							// En la punta el eje y la normal son paralelos -> cos = 1 -> avanza.
							// En el flanco son perpendiculares -> cos = 0 -> NO avanza.
							// La localizacion apical sale de la CINEMATICA, no de un umbral.
							//
							// Esto SUSTITUYE al viejo factor tipSharp (d.n)^(p-1), que era un
							// afilado ad-hoc encima de una direccion axial: hacia aproximadamente
							// lo correcto por aproximadamente la razon equivocada. m_deliveryPow > 1
							// permite exagerarlo, pero el valor defendible es q = 1 (la identidad).
							number _cos = 0.0;
							{
								number _nn = VecLength(vLevelSetGrad[ip]) + 1e-9;
								number _dn = VecLength(vDirection[ip]) + 1e-9;
								_cos = VecDot(vDirection[ip], vLevelSetGrad[ip]) / (_nn * _dn);
								if (_cos < 0.0) _cos = 0.0;   // el transporte que se aleja no empuja
								magnitud *= (m_deliveryPow == 1.0) ? _cos : pow(_cos, m_deliveryPow);
							}

							// --- FACTOR 2: MADURACION DE LA PARED ----------------------------
							// blandura(edad) = 1 / (1 + (edad/tau)^m)
							// de Jong, Hulshof & Prokert (2020, J Theor Biol 496:110253) DEMUESTRAN
							// que NO EXISTE solucion estacionaria de crecimiento apical con
							// extensibilidad constante: hace falta mu(Psi) creciente con mu -> inf.
							// Fayant et al. (2010, Plant Cell 22:2579) lo confirman numericamente:
							// modulo de Young uniforme => la celula se hincha como ESFERA, sin tubo.
							// Es decir, la maduracion no es un adorno: es condicion NECESARIA para
							// que exista un tubo que crece manteniendo su diametro.
							// Ejemplo de de Jong: mu(Psi) = 1 + Psi^3  =>  blandura = 1/(1+Psi^3),
							// que es este mismo factor con m = 3.
							// Sin campo de edad conectado, o con tau<=0, vale 1 (sin maduracion).
							if (m_matTau > 0.0 && m_spAge.valid())
							{
								number _age = vAge[ip];
								if (_age < 0.0) _age = 0.0;
								number _r = _age / m_matTau;
								number _stiff = (m_matPow == 1.0) ? _r : pow(_r, m_matPow);
								magnitud *= 1.0 / (1.0 + _stiff);
							}

							// --- FACTOR 3: REGULARIZADOR DE CURVATURA  v_n -= gamma*kappa ------
							// Sethian (2001) JCP: para F = 1 - eps*kappa la curvatura obedece
							//     kappa_t = eps*kappa_aa + eps*kappa^3 - kappa^2
							// donde el termino de REACCION empuja a la singularidad y el de DIFUSION
							// (eps*kappa_aa) la frena. Con eps = 0 queda kappa_t = -kappa^2, cuya
							// solucion kappa(t) = kappa_0/(1+t*kappa_0) es SINGULAR EN TIEMPO FINITO
							// alli donde la curvatura inicial es negativa.
							// Un gate de curvatura es una funcion DISCONTINUA de kappa: no aporta
							// nada de esa difusion, o sea equivale a correr con eps = 0.
							// Ademas un gate que dispara en puntas convexas y empuja hacia fuera es
							// flujo por curvatura media INVERTIDO: mal planteado en el sentido de
							// Hadamard (una perturbacion de numero de onda q crece como +gamma*q^2),
							// asi que gana siempre la frecuencia mas alta que la malla admita
							// -> refinar EMPEORA. Coincide con lo medido: numRef6 salio 2.7x peor.
							// El termino -gamma*kappa devuelve el signo estabilizador correcto.
							// OJO: hace la ecuacion parabolica => el paso de tiempo pasa a ir como
							// dx^2, no dx. Con gamma = 0 queda desactivado.
							if (m_curvReg > 0.0)
								magnitud -= m_curvReg * vCurvature[ip];
									  ...
									  const MathMatrix<refDim, dim> *vJT = NULL) const
			{
				...
				const number *vAge = m_spAge.valid() ? m_spAge->values(s) : NULL;
		...
		public:
			...
			// ---------------------------------------------------------------------------
			// [TIP GROWTH 20260719]  Parametros de los dos mecanismos biologicos
			// ---------------------------------------------------------------------------
			///	campo de EDAD de la membrana (opcional; sin el, no hay maduracion)
			void set_Age(SmartPtr<CplUserData<number, dim>> data)
			{
				m_spAge = data;
				m_spDAge = data.template cast_dynamic<DependentUserData<number, dim>>();
				base_type::set_input(_AGE_, data, data);
			}
			void set_Age(number val) { set_Age(make_sp(new ConstUserNumber<dim>(val))); }

			/// tiempo de maduracion: blandura = 1/(1 + edad/tau).  tau<=0 desactiva.
			void set_maturation_time(number tau) { m_matTau = tau; }

			/// exponente de la ENTREGA: v ~ (dir.n)^q con q>=1. q=1 = entrega pura.
			void set_delivery_power(number q) { m_deliveryPow = q; }

			/// exponente de la maduracion: blandura = 1/(1+(edad/tau)^m). de Jong usa m=3.
			void set_maturation_power(number m) { m_matPow = m; }

			/// regularizador de curvatura: v_n -= gamma*kappa. Devuelve el signo estabilizador.
			void set_curvature_regularisation(number g) { m_curvReg = g; }

			/// 1 = conserva el gate de curvatura (comparacion); 0 = SIN gate (modo biologico)
			void set_use_curvature_gate(bool b) { m_useGate = b; }
		...
		protected:
			...
			// [TIP GROWTH 20260719] EDAD de la membrana (opcional).
			// Campo advectado con la interfaz que mide cuanto tiempo lleva ese trozo de
			// membrana existiendo. En biologia la pared es extensible SOLO cerca del apice
			// y se rigidiza detras: por eso una raiz o una hifa crece FINA y no se infla.
			// Si no se conecta, m_spAge se queda nulo y la blandura vale 1 (sin maduracion).
			static const size_t _AGE_ = 10;
			SmartPtr<CplUserData<number, dim>> m_spAge;
			SmartPtr<DependentUserData<number, dim>> m_spDAge;
			...
			// [TIP GROWTH 20260719] parametros de los dos mecanismos biologicos
			number m_matTau      = -1.0;   // maduracion de la pared: blandura = 1/(1+edad/tau). <=0 = desactivada
			number m_deliveryPow =  1.0;   // exponente de cos(theta); 1.0 = la identidad exacta
			number m_matPow      =  1.0;   // exponente de la maduracion; de Jong usa 3
			number m_curvReg     =  0.0;   // gamma del regularizador -gamma*kappa; 0 = desactivado
			bool   m_useGate     = false;  // false = SIN gate de curvatura (modo biologico); true = comparar con el viejo
